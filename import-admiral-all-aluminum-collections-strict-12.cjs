/*
  Admiral Outdoor - STRICT 12 aluminum collection importer
  ...
  Output:
    admiral-all-aluminum-collections-strict-12-import.json
*/
...
const OUTPUT_FILE = "admiral-all-aluminum-collections-strict-12-import.json";
...
function findSpecValue($, labels) {
  ...
  // 1) Look through table rows first.
  let value = "";
  $("tr").each((_, row) => {
    if (value) return;
    ...
    if (cells.length >= 2) {
      ...
      if (wanted.includes(label)) value = cells.slice(1).join(" ");
    }
  });

  if (value) return value;

  // 2) Look at lines of page text like "Seat Height: 14".
  const bodyText = cleanText($("body").text());
  for (const label of labels) {
    const escaped = label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const regex = new RegExp(`${escaped}\\s*:?\\s*([^|\\n;]+?)(?=\\s+(SKU|Material|Dimensions|Total Weight|Seat Height|Seat Depth|Seat Width|Arm Height|Stackable)\\s*:?|$)`, "i");
